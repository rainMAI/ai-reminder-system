...
# ============================================================================
# Eye Display Service - 眼睛显示服务
# ============================================================================

# 导入眼睛显示路由
try:
    from routes.eye_display_routes import bp as eye_display_bp, init_eye_display_routes
    EYE_DISPLAY_AVAILABLE = True
    print("[OK] Eye Display module loaded")
except ImportError as e:
    EYE_DISPLAY_AVAILABLE = False
    print(f"[ERROR] Failed to load Eye Display module: {e}")
    import traceback
    traceback.print_exc()

# 注册眼睛显示路由
if EYE_DISPLAY_AVAILABLE:
    app.register_blueprint(eye_display_bp)
    print("[OK] Eye Display routes registered")
    print("     API endpoints:")
    print("       - GET/POST /api/eye/status - 获取服务状态")
    print("       - POST /api/eye/expression - 设置表情")
    print("       - POST /api/eye/animation - 播放动画")
    print("       - POST /api/eye/text - 显示文字")
    print("       - POST /api/eye/brightness - 设置亮度")
    print("       - POST /api/eye/happy - 开心表情")
    print("       - POST /api/eye/sad - 伤心表情")
    print("       - POST /api/eye/angry - 生气表情")
    print("       - POST /api/eye/surprised - 惊讶表情")
    print("       - POST /api/eye/neutral - 中性表情")
    print("       - POST /api/eye/thinking - 思考表情")
    print("       - POST /api/eye/listening - 聆听表情")
    print("       - POST /api/eye/blink - 眨眼动画")
    print("       - POST /api/eye/talk - 说话动画")
    print("       - POST /api/eye/idle - 空闲动画")
    print("       - POST /api/eye/test - 测试所有功能")
...
# Static files - Generic route for all static files
...
def main():
    ...
    # 注册对话管理路由
    try:
        from routes.chat_manager_routes import register_chat_manager_routes
        register_chat_manager_routes(app)
        print("[OK] Chat Manager routes registered")
    except ImportError as e:
        print(f"[ERROR] Failed to load chat manager routes: {e}")

    # 注册用户设备路由
    try:
        from routes.user_device_routes import register_user_device_routes
        register_user_device_routes(app)
        print("[OK] User Device routes registered")
    except ImportError as e:
        print(f"[ERROR] Failed to load user device routes: {e}")
    ...
    if EYE_DISPLAY_AVAILABLE:
        try:
            # 从环境变量读取配置
            eye_port = os.environ.get('EYE_DISPLAY_PORT', 'COM3')
            eye_baudrate = int(os.environ.get('EYE_DISPLAY_BAUDRATE', '115200'))
            eye_enabled = os.environ.get('EYE_DISPLAY_ENABLED', 'true').lower() == 'true'

            if eye_enabled:
                print(f"[INFO] 正在启动眼睛显示服务: {eye_port}@{eye_baudrate}")
                eye_service = init_eye_display_routes(eye_port, eye_baudrate)
                if eye_service:
                    print("[OK] 眼睛显示服务已启动")
                    print(f"     串口: {eye_port}")
                    print(f"     波特率: {eye_baudrate}")
                else:
                    print("[WARNING] 眼睛显示服务启动失败，将继续运行但眼睛功能不可用")
            else:
                print("[INFO] 眼睛显示服务已禁用 (EYE_DISPLAY_ENABLED=false)")
        except Exception as e:
            print(f"[ERROR] 眼睛显示服务启动失败: {e}")
            import traceback
            traceback.print_exc()
